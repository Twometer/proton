...
/**
 * Copied from Procyon with minor modifications,
 * as I could not just extend it because someone made it
 * "final".
 */
public class JavaOutputVisitorEx implements IAstVisitor<Void, Void> {
    ...
    private final TextOutputFormatter formatter;
    private final DecompilerSettings settings;
    private final JavaFormattingOptions policy;
    private final Stack<AstNode> containerStack = new Stack<>();
    private final Stack<AstNode> positionStack = new Stack<>();
    private final ITextOutput output;
    private JavaOutputVisitorEx.LastWritten lastWritten;

    public JavaOutputVisitorEx(final ITextOutput output, final DecompilerSettings settings) {
        this.output = output;
        ...
        this.formatter = new TextOutputFormatter(
                ...
                TextOutputFormatter.LineNumberMode.WITH_DEBUG_LINE_NUMBERS : TextOutputFormatter.LineNumberMode.WITHOUT_DEBUG_LINE_NUMBERS
        );
    ...
    private static boolean isKeyword(final String identifier) {
        return ArrayUtilities.contains(KEYWORDS, identifier);
    }

    @SuppressWarnings("UnusedParameters")
    private static boolean isKeyword(final String identifier, final AstNode context) {
        return ArrayUtilities.contains(KEYWORDS, identifier);
    }
    ...
    private void startNode(final AstNode node) {
    ...
    private void endNode(final AstNode node) {
    ...
    private void space() {
        formatter.space();
        lastWritten = JavaOutputVisitorEx.LastWritten.Whitespace;
    }

    private void space(final boolean addSpace) {
        if (addSpace) {
            space();
        }
    }
    ...
    private void openBrace(final BraceStyle style) {
        writeSpecialsUpToRole(Roles.LEFT_BRACE);

        space(
                (style == BraceStyle.EndOfLine || style == BraceStyle.BannerStyle) &&
                        lastWritten != JavaOutputVisitorEx.LastWritten.Whitespace && lastWritten != JavaOutputVisitorEx.LastWritten.LeftParenthesis
        );

        formatter.openBrace(style);

        lastWritten = style == BraceStyle.BannerStyle ? JavaOutputVisitorEx.LastWritten.Other
                : JavaOutputVisitorEx.LastWritten.Whitespace;
    }

    private void closeBrace(final BraceStyle style) {
        writeSpecialsUpToRole(Roles.RIGHT_BRACE);
        formatter.closeBrace(style);
        lastWritten = JavaOutputVisitorEx.LastWritten.Other;
    }
    ...
    private void writeIdentifier(final String identifier, final Role<Identifier> identifierRole) {
        writeSpecialsUpToRole(identifierRole != null ? identifierRole : Roles.IDENTIFIER);

        if (isKeyword(identifier, containerStack.peek())) {
            if (lastWritten == JavaOutputVisitorEx.LastWritten.KeywordOrIdentifier) {
                space();
            }
            // this space is not strictly required, so we call space()
//            formatter.writeToken("$");
        } else if (lastWritten == JavaOutputVisitorEx.LastWritten.KeywordOrIdentifier) {
            formatter.space();
        ...
        lastWritten = JavaOutputVisitorEx.LastWritten.KeywordOrIdentifier;
    }

    private void writeToken(final TokenRole tokenRole) {
        writeToken(tokenRole.getToken(), tokenRole);
    }

    private void writeToken(final String token, final Role role) {
        writeSpecialsUpToRole(role);

        if (lastWritten == JavaOutputVisitorEx.LastWritten.Plus && token.charAt(0) == '+' ||
                lastWritten == JavaOutputVisitorEx.LastWritten.Minus && token.charAt(0) == '-' ||
                lastWritten == JavaOutputVisitorEx.LastWritten.Ampersand && token.charAt(0) == '&' ||
                lastWritten == JavaOutputVisitorEx.LastWritten.QuestionMark && token.charAt(0) == '?' ||
                lastWritten == JavaOutputVisitorEx.LastWritten.Division && token.charAt(0) == '*') {
        ...
        if (role instanceof TokenRole) {
            ...
            if (tokenRole.isKeyword()) {
                ...
                lastWritten = JavaOutputVisitorEx.LastWritten.Operator;
                return;
            } else if (tokenRole.isDelimiter()) {
                formatter.writeDelimiter(token);
                lastWritten = "(".equals(token) ? JavaOutputVisitorEx.LastWritten.LeftParenthesis : JavaOutputVisitorEx.LastWritten.Delimiter;
                return;
            }
        }
        ...
        switch (token) {
            case "+":
                lastWritten = JavaOutputVisitorEx.LastWritten.Plus;
                break;
            case "-":
                lastWritten = JavaOutputVisitorEx.LastWritten.Minus;
                break;
            case "&":
                lastWritten = JavaOutputVisitorEx.LastWritten.Ampersand;
                break;
            case "?":
                lastWritten = JavaOutputVisitorEx.LastWritten.QuestionMark;
                break;
            case "/":
                lastWritten = JavaOutputVisitorEx.LastWritten.Division;
                break;
            case "(":
                lastWritten = JavaOutputVisitorEx.LastWritten.LeftParenthesis;
                break;
            default:
                lastWritten = JavaOutputVisitorEx.LastWritten.Other;
                break;
    ...
    private void comma(final AstNode nextNode) {
        comma(nextNode, false);
    }

    private void comma(final AstNode nextNode, final boolean noSpaceAfterComma) {
...
*/
    ...
    private void writeTypeParameters(final Iterable<TypeParameterDeclaration> typeParameters) {
    ...
    private void writeEmbeddedStatement(final Statement embeddedStatement) {
    ...
    private void writeMethodBody(final AstNodeCollection<TypeDeclaration> declaredTypes, final BlockStatement body) {
        if (body.isNull()) {
            semicolon();
            return;
        }
...
//        }
        ...
        if (declaredTypes != null && !declaredTypes.isEmpty()) {
            for (final TypeDeclaration declaredType : declaredTypes) {
                ...
                declaredType.acceptVisitor(new JavaOutputVisitorEx(output, settings), null);
                needNewLine = true;
            }
        }
    ...
    private void writeAnnotations(final Iterable<Annotation> annotations, final boolean newLineAfter) {
    ...
    private void writePrivateImplementationType(final AstType privateImplementationType) {
    ...
    private void writeKeyword(final TokenRole tokenRole) {
        writeKeyword(tokenRole.getToken(), tokenRole);
    }

    private void writeKeyword(final String token) {
        writeKeyword(token, null);
    }

    private void writeKeyword(final String token, final Role tokenRole) {
        if (tokenRole != null) {
    ...
    private void visitNodeInPattern(final INode childNode) {
    ...
    public Void visitComment(final Comment comment, final Void ignored) {
        if (lastWritten == JavaOutputVisitorEx.LastWritten.Division) {
            formatter.space();
        }
    ...
    public Void visitDoWhileStatement(final DoWhileStatement node, final Void ignored) {
        ...
        space(lastWritten != JavaOutputVisitorEx.LastWritten.Whitespace);
    ...
    private void writePrimitiveValue(final Object val) {
        if (val == null) {
            writeKeyword("null");
            return;
        }
    ...
    public Void visitAnonymousObjectCreationExpression(final AnonymousObjectCreationExpression node, final Void ignored) {
        ...
        space(policy.SpaceBeforeMethodCallParentheses);
        writeCommaSeparatedListInParenthesis(node.getArguments(), policy.SpaceWithinMethodCallParentheses);
        node.getTypeDeclaration().acceptVisitor(new JavaOutputVisitorEx(output, settings), ignored);
        endNode(node);
        return null;
    }
